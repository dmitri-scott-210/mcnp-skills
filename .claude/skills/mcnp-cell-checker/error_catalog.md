# MCNP Cell Checker - Error Catalog

Complete catalog of common cell card problems with diagnostics, root causes, and solutions.
...
**Symptoms**:
```
WARNING: Universe 50 defined multiple times
         Found in cells: 500, 520, 540
```
...
for cell in cells:
    if 'u' in cell.parameters:
        u = cell.parameters['u']
        if u in universe_definitions:
            universe_definitions[u].append(cell.number)
        else:
            universe_definitions[u] = [cell.number]

for u, cells in universe_definitions.items():
    if len(cells) > 1:
        warning(f"Universe {u} defined in multiple cells: {cells}")
```

**Root Causes**:
1. Copy-paste error
2. Forgot to change universe number
3. Intentional (multiple cells in same universe)

**Solutions**:

```
BAD: Accidental duplicate (different geometry)
  500 1 -10.5 -500 u=50 imp:n=1    ✗ Fuel pellet
  520 2 -6.5 -520 u=50 imp:n=1     ✗ Cladding (should be u=51)

GOOD: Unique universe numbers
  500 1 -10.5 -500 u=50 imp:n=1    ✓ Fuel pellet (u=50)
  520 2 -6.5 -520 u=51 imp:n=1     ✓ Cladding (u=51)

GOOD: Intentional (same geometry, multiple instances)
  500 1 -10.5 -500 u=50 imp:n=1    ✓ Pin instance 1
  501 1 -10.5 -501 u=50 imp:n=1    ✓ Pin instance 2 (OK, same u)
```
...
**Symptoms**:
```
WARNING: 3 universes defined but never used
         Unused: [300, 350, 400]
```

**Diagnosis**:
```python
defined = set(u for cell in cells if 'u' in cell.parameters
              for u in [cell.parameters['u']])
used = set(u for cell in cells if 'fill' in cell.parameters
           for u in extract_fill_universes(cell.parameters['fill']))

unused = defined - used
if unused:
    warning(f"Unused universes: {unused}")
```

**Root Causes**:
1. Deleted fill reference but kept definition
2. Commented out lattice using this universe
3. Work-in-progress/incomplete model

**Solutions**:

```
Option 1: Remove unused definitions (if truly not needed)
  c 300 1 -2.7 -300 u=300 imp:n=1    ✓ Commented out

Option 2: Add fill reference (if should be used)
  200 0 -200 lat=1 fill=-3:3 -3:3 0:0 imp:n=1
      ...
      300 300 300    ✓ Now using u=300

Option 3: Keep for future use (add comment)
  c Universe 300: Reserved for future control rod insertion
  300 1 -2.7 -300 u=300 imp:n=1    ✓ Defined but not yet used
```
