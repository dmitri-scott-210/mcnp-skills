...
PATH = os.getcwd()
NAME = "verification keff"
BENCHMARKS_DIR = "problems"
...
def collect_benchmark_results(benchmarks, results, label, get_uncertainty=True):
    ...
    if get_uncertainty:
        data[unc] = [
            benchmark.info[results]["k-eff"]["std"] for benchmark in benchmarks
        ]
...
def post_calc(calc_name):
    ...
    for benchmark in benchmarks:
        ...
        # TODO   get data used in this calculation, e.g., ENDF8, ENDF71, etc.
...
def doc_calc(calc_name, use_latex, compare_calcs):
    ...
    e_data, e_map = collect_benchmark_results(
        benchmarks, "analytic_data", "Analytic", get_uncertainty=False
    )
    formatting = {
        e_map["val"]: FixedPoint(6),
    }
    ...
    for c_path in calc_paths:
        ...
        ce_val, ce_unc, _ = vnv.c_over_b(
            data[map["val"]], data[map["unc"]], e_data[e_map["val"]]
        )

        ce_map_entry = copy.copy(map)
        ce_map_entry["val"] = calc_name + " k-eff C/E"
    ...
    all_plot.set_dependent_label(
        LatexString("k-effective C/E +/- 1σ", r"$k_{\textrm{eff}}$ $C/E \pm 1\sigma$")
    )
    ...
    prob_type = set(cbdata.df[0]["Problem Type"])
    for type in prob_type:
        type_df = [df[df["Problem Type"] == type] for df in cbdata.df]
        plot_files.append("{}_results.png".format(type.replace(" ", "_")))
        plot_scaling.append(0.87)

        type_plot = vnv.plotndoc.ResultPlot(
            (6.5, 10), use_latex=use_latex, transpose=True
        )

        indep_label = type_df[0].index.array

        for data, map in zip(type_df, cbdata.vumap):
            dep_data = data[map["val"]].to_numpy()
            dep_err = data[map["unc"]].to_numpy()
            indep_data = range(len(data.index))

            type_plot.plot_discrete(
                indep_data, dep_data, dep_err=dep_err, label=map["val"]
            )

        type_plot.set_independent_label("Benchmarks")
        type_plot.set_dependent_label(
            LatexString(
                "k-effective C/E +/- 1σ", r"$k_{\textrm{eff}} $ $ C/E \pm 1\sigma$"
            )
        )
        type_plot.set_independent_tick_labels(indep_label, 90)
        type_plot.add_zebrastripe()
        type_plot.add_grid(dep_grid=True, indep_grid=False)
        if len(calc_paths) <= 1:
            type_plot.legend = False
        type_plot.save(os.path.join(docs_path, plot_files[-1]), legend_ncol=2)
